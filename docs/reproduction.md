...
The validation figures are generated by the main paper-figure module from
...
This writes 600-dpi PNG and TIFF rasters plus vector PDFs and plot-data CSVs to:

```text
paper/figures/validation/
```

The existing `paper/figures/figure_manifest.csv` remains the manifest for
figures used directly in the manuscript. The validation figure artifacts are
tracked in `paper/final_paper_manifest.csv`, including hashes and source
provenance, without requiring a second figure manifest.
